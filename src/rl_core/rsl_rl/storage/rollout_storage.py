...
from src.rl_core.rsl_rl.modules import HiddenState
from src.rl_core.rsl_rl.utils import split_and_pad_trajectories
...
class RolloutStorage:
    ...
    class Batch:
        ...
        def __init__(
            ...
            self.old_distribution_params: tuple[torch.Tensor, ...] | None = (
    ...
    def __init__(
        ...
        if training_type == "rl":
            ...
            self.distribution_params: tuple[torch.Tensor, ...] | None = (
                None  # Lazily initialized on first transition
            )
    ...
    def add_transition(self, transition: Transition) -> None:
        ...
        if self.training_type == "rl":
            ...
            if (
                self.distribution_params is None
            ):  # Initialize the distribution parameters
                self.distribution_params = tuple(
                    torch.zeros(
                        self.num_transitions_per_env, *p.shape, device=self.device
                    )
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                if self.saved_hidden_state_a is not None:
                    ...
                    hidden_state_a_batch = (
                        hidden_state_a_batch[0]
                        if len(hidden_state_a_batch) == 1
                        else hidden_state_a_batch
                    )
                else:
                ...
                if self.saved_hidden_state_c is not None:
                    ...
                    hidden_state_c_batch = (
                        hidden_state_c_batch[0]
                        if len(hidden_state_c_batch) == 1
                        else hidden_state_c_batch
                    )
                else:
    ...
    def _save_hidden_states(
        self, hidden_states: tuple[HiddenState, HiddenState]
    ) -> None:
