...
from src.rl_core.rsl_rl.utils import unpad_trajectories

HiddenState = Union[
    torch.Tensor, tuple[torch.Tensor, torch.Tensor], None
]  # Using Union due to Python <3.10
    ...
    def forward(
        ...
        if batch_mode:
            ...
            if hidden_state is None:
                raise ValueError(
                    "Hidden states not passed to RNN module during policy update"
                )
            ...
            if dones is None:  # Detach hidden state
                if isinstance(self.hidden_state, tuple):  # Tuple in case of LSTM
                    self.hidden_state = tuple(
                        hidden_state.detach() for hidden_state in self.hidden_state
                    )
                else:
                ...
                if isinstance(self.hidden_state, tuple):  # Tuple in case of LSTM
                    ...
                    self.hidden_state[..., dones == 1, :] = self.hidden_state[
                        ..., dones == 1, :
                    ].detach()
