...
from src.rl_core.rsl_rl.utils import get_param, resolve_nn_activation
    ...
    def __init__(
        ...
        for idx in range(len(output_channels)):
            ...
            if n == "none":
                ...
                layers.append(
                    nn.LayerNorm(
                        [output_channels[idx], norm_input_dim[0], norm_input_dim[1]]
                    )
                )
            else:
                raise ValueError(
            ...
            last_dim = _compute_output_dim(
                last_dim, k, s, d, p, is_max_pool=get_param(max_pool, idx)
            )
        ...
        self._output_dim = (
            last_dim if not flatten else last_channels * last_dim[0] * last_dim[1]
        )
...
def _compute_padding(
    input_hw: tuple[int, int], kernel: int, stride: int, dilation: int
) -> tuple[int, int]:
    ...
    h = math.ceil(
        (
            stride * math.floor(input_hw[0] / stride)
            - input_hw[0]
            - stride
            + dilation * (kernel - 1)
            + 1
    ...
    w = math.ceil(
        (
            stride * math.floor(input_hw[1] / stride)
            - input_hw[1]
            - stride
            + dilation * (kernel - 1)
            + 1
