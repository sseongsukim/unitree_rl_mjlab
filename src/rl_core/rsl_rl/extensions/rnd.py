...
from src.rl_core.rsl_rl.modules import (
    MLP,
    ...
    def __init__(
        ...
        if state_normalization:
            self.state_normalizer = EmpiricalNormalization(
                shape=[self.num_states], until=int(1.0e8)
            ).to(self.device)
        else:
...
def resolve_rnd_config(
    alg_cfg: dict, obs: TensorDict, obs_groups: dict[str, list[str]], env: VecEnv
) -> dict:
