...
class Logger:
    ...
    def __init__(
        ...
        if self.cfg["algorithm"]["rnd_cfg"]:
            ...
            self.cur_ereward_sum = torch.zeros(
                self.num_envs, dtype=torch.float, device=self.device
            )
            self.cur_ireward_sum = torch.zeros(
                self.num_envs, dtype=torch.float, device=self.device
            )
    ...
    def init_logging_writer(self) -> None:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from src.rl_core.rsl_rl.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(
                    log_dir=self.log_dir, flush_secs=10, cfg=self.cfg
                )
            elif self.logger_type == "wandb":
                from src.rl_core.rsl_rl.utils.wandb_utils import WandbSummaryWriter
                ...
                raise ValueError(
                    "Logger type not found. Please choose 'wandb', 'neptune', or 'tensorboard'."
                )
        else:
            self.writer = None
    ...
    def log(
        ...
        if self.writer is not None:
            ...
            if len(self.rewbuffer) > 0:
                if self.cfg["algorithm"]["rnd_cfg"]:
                    self.writer.add_scalar(
                        ...
                        statistics.mean(self.erewbuffer),
                        it,
                    )
                    self.writer.add_scalar(
                        "Rnd/mean_intrinsic_reward",
                        statistics.mean(self.irewbuffer),
                        it,
                    )
                ...
                self.writer.add_scalar(
                    "Train/mean_episode_length", statistics.mean(self.lenbuffer), it
                )
                if self.logger_type != "wandb":
                    self.writer.add_scalar(
                        "Train/mean_reward/time",
                        statistics.mean(self.rewbuffer),
                        int(self.tot_time),
                    )
                    self.writer.add_scalar(
            ...
            # Print std
            log_string += (
                f"""{"Mean action std:":>{pad}} {action_std.mean().item():.2f}\n"""
            )
    ...
    def _store_code_state(self) -> list[str]:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            for repository_file_path in self.git_status_repos:
                try:
                    repo = git.Repo(
                        repository_file_path, search_parent_directories=True
                    )
                    t = repo.head.commit.tree
                    commit_hash = repo.head.commit.hexsha
                except Exception:
                    print(
                        f"Could not find git repository in {repository_file_path}. Skipping."
                    )
                    continue
