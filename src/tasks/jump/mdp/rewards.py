...
def obstacle_approach_speed(
  ...
  speed = torch.clamp(forward_speed, min=0.0, max=target_speed) / max(target_speed, 1.0e-6)
  return active * speed
...
def distance_to_clear_goal(
  ...
  asset: Entity = env.scene[asset_cfg.name]
  distance = torch.clamp(goal_x - asset.data.root_link_pos_w[:, 0], min=0.0, max=clamp_distance)
